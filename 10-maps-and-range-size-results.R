...
##     to pull the associated voronoi polygon map to vizualize what this value
  ...
  imap(function(df.x, stat) {
    ...
    p_c <- ggplot(df.x %>% filter(scenario == "current\n")) +
      stat_summary_hex(aes(y = LAT_ACTUAL, x = elev_km, z = value),
                       fun = ~ mean(.x),
                       bins = 40) +
      facet_grid(spp_common ~ scenario, scales = "free") +
...
# Notes
# - these figures just for my own inspection; Appendix C Figures for the VP
#   method are produced by script 11-...R
...
#   future climate scenario and the current scenario (as c - f).
# - these numbers correspond to the top panel of Table C1 (appendix C) for
#   the VP method of range estimation.
range_size_key %>%
  map(function(sp.x) {
   sp.x %>%
      filter(map_group == "maps_2k") %>%
