...
## 
## About - this script:
## - Calculate summary statistics based on prediction maps
##
## About - output:
...
# == Load data & fns (FIX once sdm finalized) ============================ ####
# - fns & drop plot ids -------------------------------------------------- ####
# load fns
...
# - clim_vars; data_spp; data_spp.std; parms_start_spp
# - see what we can delete from the above later on....
...
# - make crosswalk with longer variable names for figures
long_name_var <- data.frame(
  ...
  units = c("days",
            ...
            "C",
            "")) %>%
  mutate(both_names = paste0(long_names, " (", orig_names, "; ", units, ")"))
long_name_var[long_name_var$orig_names == "intercept", ]$both_names <- "FE intercept"
long_name_var[long_name_var$orig_names == "gdd5", ]$both_names <- "growing degree days"
...
# == Color palette and figure arguments ================================== ####
scen_colPal <- data.frame(
  ...
  # color blind friendly with reds for Had and blues for CCSM4
  hex_cbsafe_br = c("#999999",
                    "#56B4E9", "#0072B2",
                    "#E69F00", "#D55E00"),
  # another cb safe with purple and greens
  hex_cbsafe_gp = c("#999999",
                    "#762a83", "#af8dc3",
                    "#1b7837", "#7fbf7b"))

# set general text-size & figure width arguments
# - these based on Ecological Applications' author guidelines
EA_axis_text_size  <- 6 # 6-7
EA_axis_title_size <- 8 # 8-9
EA_strip_text_size <- 7 # 7-8
EA_fig_width_max  <- 6 #in
EA_fig_ht_max     <- 8 #in
EA_fig_width_1col <- 3 #in
EA_width_unit <- "in"
EA_dpi <- 800
...
p_body <- (mod_spp %>% 
  ...
  ylab("value") +
  theme_bw() +
  theme(strip.text.x = element_text(size = EA_strip_text_size),
        # strip.text.y = element_text(size = EA_strip_text_size),
        axis.title.y = element_text(size = EA_axis_title_size),
        axis.title.x = element_text(size = EA_axis_title_size),
        axis.text.x  = element_text(size = EA_axis_text_size),
        axis.text.y  = element_text(size = EA_axis_text_size),
        # legend.title = element_text(size = EA_axis_title_size),
        # legend.text  = element_text(size = EA_strip_text_size),
...
ggsave(filename = paste0(path_images, 
                         "FigS8.tiff"),
       plot = p_body,
       height = EA_fig_width_max, 
       width = EA_fig_width_max,
       units = EA_width_unit, 
       dpi = EA_dpi,
       compression = "lzw")

# - Section S4 Figure S7: betas, sigma_z, sigma_v, rho ------------------- ####
p_body <- list(mod_spp %>% map(~.x$MCMC_theta), beta_crosswalk) %>%
  pmap(function(theta, cross) {
    # get bio-style names (e.g. bio10_bio18, etc)
    names(theta)[match(cross$mod_name, names(theta))] <- cross$orig_names
  ...
  map(function(theta.x) {
    (theta.x %>% 
       bind_rows(.id = "spp") %>%
       mutate(spp = toupper(spp)) %>%
       mutate(spp = factor(spp, levels = rev(toupper(names(mod_spp)))))) %>%
      ggplot() +
...
list(mod_spp, beta_crosswalk) %>% 
  pmap(function(mod.x, crosswalk.x) {
    ...
    names(post_mean)[update_index] <- crosswalk.x$orig_names
...
# Note on units:
# - Although documentation from original queried data file says ppt-vars are
#   in cm, they're actually in mm (the original units used in the Norm81m data
#   set from which these vars are derived). The calculated variables for the 
#   ClimateNA datasets are also in mm for ppt vars.
...
# View the plots
plots_spclimrel <- cond_plots %>% imap(function(ls.x, sp_name.x) {
    # - pull out the plots we need to arrange ----------------------- ####
    top.x <- ls.x %>% 
      map(~.x$p1_meanpred + 
            theme(axis.title.y = element_text(size = EA_axis_title_size),
                  axis.title.x = element_text(size = EA_axis_title_size),
                  axis.text.x  = element_text(size = EA_axis_text_size),
                  axis.text.y  = element_text(size = EA_axis_text_size),
                  legend.title = element_text(size = EA_axis_title_size),
    ...
    # create the joint title
    p_title <- ggdraw() +
      draw_label(label = paste0(sp_name.x, ": mean & optimal sp-climate relationships"),
                 x = 0,
                 hjust = 0) +
      theme(plot.margin = margin(0, 0, 2, 7))
    
    # return titled plot
    plot_grid(p_title, plot_body,
              ncol = 1,
              rel_heights = c(0.1, 1))
  })
...
p_body %>%
  list(.,
       # trial and error, best sizes
       c(2.03, 3.35, 3.35, 3.15, 3.35),
       c(5.5,  6,    6,    6,    6),
       # c(1.75, 3.07, 3.07, 3.15, 3.07),
       # c(4.75, 5.5, 5.5, 6, 5.5),
...
ggsave(filename = paste0(path_images, "tmp.tiff"),
       plot = p_body[[1]],
       height = 1.75, 
       width = 4.75,
       units = "in", 
...
p_body <- ggplot() +
  ...
  theme_void() + 
  theme(legend.title = element_text(size = EA_axis_title_size),
        legend.text  = element_text(size = EA_strip_text_size))
