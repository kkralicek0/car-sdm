...
# - BKDE's PEC table (Appendix C, Table C3) ------------------------------ ####
    ...
    p_range_est <- (df.x %>% 
      ...
      theme(plot.margin = abpr_margin,
            axis.title.x = element_text(vjust = 3, 
                                        size = EA_axis_title_size),
            # legend.title = element_blank(),
            strip.text.x = element_text(size = EA_strip_text_size),
...
(kde_future_diff %>%
  ...
  scale_fill_manual(values = unname((with(scen_colPal, 
                                          hex_cb_gp %>% 
                                            setNames(scenario)))[2:5]))  +
...
bkde_boxplot.df <- kde_future_diff %>%
  ...
  imap(function(df.x, sp_name.x) {
    df.x %>% 
      ...
      ungroup() %>%
      mutate(spp = toupper(sp_name.x)) %>%
      mutate(yint = ifelse(spp == "ABPR", NA, 0))
  }) %>%
  bind_rows()
...
# - VP: change & rel-change (Appendix C, Figure C1) ---------------------- ####
...
# - Range-size estimates (Appendix C, Table C2) -------------------------- ####
...
# - change & relative-change in range-size (Appendix C, Table C1) -------- ####
