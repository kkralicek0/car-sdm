...
fn.cond_spclim_rel <- function(MCMC_theta.x, dat_ls.x, dat_std.x, covar_lims.x, 
  ...
  # - `long_names = TRUE` will set plot axis names to their long format
  #   (e.g. 'ppt in wettest qtr (bio16; cm)', etc.)
    ...
    cond_plots[1:length(inter_base)] <- lapply(inter_base, function(x) {
      ...
      p1_meanpred <- ggplot(df_plot.x) +
        geom_tile(aes_string(x = x_var, y = y_var, fill = z_var),
                  alpha = 0.9) +
        scale_fill_gradient("probabilty",
...
fn.map_mp_uncert <- function(preds.x, sp_name, plot_sf.x,
  ...
  if (T_fnodiff_used) {
    ...
    pe_future <- future.df %>%
      ...
      geom_sf(data = study_area.sf, fill = "grey50") +
      geom_sf(aes(fill = mu_hat), color = NA)  +
      # scale_fill_distiller("diff prob.", palette = "YlGnBu") +
    ...
    pu_future <- future.df %>%
      ...
      geom_sf(data = study_area.sf, fill = "grey50") +
      geom_sf(aes(fill = se_mu), color = NA)  +
      # scale_fill_viridis_c("diff SE", option = "cividis") +
...
fn.map_rangesize <- function(preds.x, sp_name.x, plot_sf.x, study_area.sf,
                             diff_map_T = FALSE) {
  # About:                                                                -----
  # Makes a 2x5 paneled figure, where each panel shows the map corresponding
  ...
  #   or 'TRUE' for future-maps to be visualized as the difference from the 
  #   current-map
